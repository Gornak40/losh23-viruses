...
pair<pair<ll, ll>, bool> move(ll movenum) {
    ...
    else if (vbMove.size() > 0) {
        pair<ll, ll> p = vbMove[0];
        return { p, false };
    }
    return { {-1, -1}, false };

    /*if (attack.size() > 0) {
        pair<ll, ll> p = attack[0];
        field[p.first][p.second] = -num;
        return { attack[0], true };
    }

    if (avail.size() > 0) {
        pair<ll, ll> p = avail[0];
        field[p.first][p.second] = num;
        return { avail[0], true };
    }*/
}
