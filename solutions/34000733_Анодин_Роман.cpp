...
//    };
    ...
    public:
        ...
        [[nodiscard]] bool GetValue(std::uint32_t id) const {
            return (arr[id >> 6u] >> (id & last_6_bits)) & 1u;
        }

        void SetToTrue(std::uint32_t id) {
...
//    std::uint32_t seed = 1111;
    ...
    std::uniform_real_distribution<double> dist_00_06(0, 0.6);
...
//        };
    ...
    void GetMaxNDoings(std::vector<std::int8_t> &table, std::uint16_t N, std::uint16_t M, std::uint16_t K,
                       ...
                       Utils::FastBoolArray &visited, std::set<std::uint32_t> &motion_poses,
                    ...
                    for (std::uint8_t dir = 0; dir < 8; ++dir) {
                        ...
                        if (table[sec_coords] == 1 ||
                            ...
                            if (may_motion_pos_cnt < probs_may_motion_poses.size())
                                ...
                                probs_may_motion_poses.emplace_back(0, coords);
                            ++may_motion_pos_cnt;
                            break;
                        }
                    }
                }
                if (table[coords] == 2 && !dsu.is_set_life_exists(coords))
                    queue_my_unlinked_crep.emplace(coords, 0);
                if (table[coords] >= 10)
...
//                              after_bfs_time - after_queues_adding_map_time).count() << "ms\n";
        for (std::uint32_t i = 0; i < may_motion_pos_cnt; ++i) {
            ...
            if (y >= 0) {
                if (y <= 10) probs_may_motion_poses[i].first -= y * 4;
                else probs_may_motion_poses[i].first -= y * 2;
            }
            if (z >= 0 || k >= 0) {
                if (z >= 0) probs_may_motion_poses[i].first += z * 2;
                if (k >= 0) probs_may_motion_poses[i].first += k;
            } else {
                probs_may_motion_poses[i].first *= 115;
                probs_may_motion_poses[i].first /= 100;
            }
        }
...
//                  << "ms\n";
        ...
        std::queue<BFSQueueElem> queue_my_unlinked_crep, queue_their_crest, queue_their_linked_crep, queue_their_unlinked_crep;
        std::vector<std::int32_t> dists_my_unlinked_crep(Utils::ZipCoordsToNum(N, M) + 1),
...
//                          after_first_init_time - after_first_input_time).count() << "ms\n";
        ...
        while (true) {
            ...
            for (std::uint32_t i = 0; i < 3 && full_doings.size() < 3; ++i) {
                GetMaxNDoings(table, N, M, K, player_id, dsu, probs_may_motion_poses, queue_my_unlinked_crep,
                              queue_their_crest, queue_their_linked_crep, queue_their_unlinked_crep,
                              dists_my_unlinked_crep, dists_their_crest, dists_their_linked_crep,
                              dists_their_unlinked_crep, visited, motion_poses, 3 - full_doings.size(), doings);
