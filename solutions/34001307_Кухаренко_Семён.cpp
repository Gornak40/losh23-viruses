...
bool is_opponent_cross(int i, int j) {
    return field[i][j] > 0 && field[i][j] != k;
}

bool is_opponent_tower(int i, int j) {
    return field[i][j] < 0 && field[i][j] != -k && field[i][j] != -100;
}
...
int main() {
    ...
    while (true) {
        ...
        fflush(stdout); cout << endl;
        string status; cin >> status;
        if (status == "o") exit(0);
        for (int __ = 0; __ < num - 1; ++__) {
            int id, ct; cin >> id >> ct;
            for (int _ = 0; _ < ct; ++_) {
                int i, j; cin >> i >> j;
                --i, --j;
                if (field[i][j] == 0) field[i][j] = id;
                else field[i][j] = -id;
            }
        }
    }
